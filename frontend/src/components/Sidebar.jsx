...
const Sidebar = () => {
    ...
    const logoutHandler = async () => {
        try {
            const res = await axios.get(`https://realtime-chat-application-8e59.onrender.com/api/v1/user/logout`);
            navigate("/login");
    ...
    return (
        <div className='border-r border-slate-800 p-4 flex flex-col'>
            <form onSubmit={searchSubmitHandler} className="flex items-center gap-0.5" action="">
                <input
                    value={search}
